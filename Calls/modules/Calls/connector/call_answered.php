<?php
	...
	///Find call by uuid in VTiger and set Status 'answered'\
	...
	if (!$login) {
		...
		if($records) {
			...
			$sql = "UPDATE vtiger_calls SET `status` = 'answered', `dst_phone` = '". $number ."'  WHERE callsid = " . $id . ";";
			if ($conn->query($sql) === true) {
				// echo "Call status updated!";
				$DB->query("UPDATE calls SET answered = 1 WHERE uuid = '". $uuid ."';");
			}
			else {
